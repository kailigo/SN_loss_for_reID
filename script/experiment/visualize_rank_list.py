...
class Config(object):
  def __init__(self):
    ...
    parser.add_argument('--dataset', type=str, default='market1501',
                        choices=['market1501', 'cuhk03', 'duke', 'cuhk03_OS_detected'])
...
def main():
  ...
  feat = [feat[id] for id in keep_idx]
  im_names = [im_names[id] for id in keep_idx]
  ids = [ids[id] for id in keep_idx]

  import pdb
  pdb.set_trace()

  np.save('temp/market_feat.npy', feat)
  np.save('temp/market_label.npy', ids)
  np.save('temp/market_im_names.npy', im_names)
