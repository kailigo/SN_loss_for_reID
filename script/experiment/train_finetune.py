...
class Config(object):
    def __init__(self):
        ...
        parser.add_argument('--dataset', type=str, default='market1501',
                            choices=['market1501', 'cuhk03', 'duke', 'combined', 'viper', 'cuhk01', 'cuhk03_OS'])
        ...
        haf = int(self.total_epochs / 3)
        tot_left = self.total_epochs - haf
        self.staircase_decay_at_epochs = range(haf, self.total_epochs, int(tot_left / 3))
        ...
        self.ckpt_file = osp.join(args.ckpt_dir, 'ckpt.pth')
...
def main():
    ...
    for ep in range(start_ep, cfg.total_epochs):
        ...
        criterion = losses.create('myknnsoftmax', alpha=cfg.knnsoftmax_alpha, k=cfg.knnsoftmax_k,
                                  weight=cfg.knnsoftmax_w).cuda()
        ...
        while not epoch_done:
            ...
            # import pdb
            # pdb.set_trace()
            loss, inter_, dist_ap, dist_an = criterion(feat.cuda(), Variable(labels_t.cuda()))
        ...
        # save ckpt
        if cfg.log_to_file:
            save_ckpt(modules_optims, ep + 1, 0, cfg.exp_dir+'./ckpt.pth')
