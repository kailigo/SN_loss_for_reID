...
def main():
  ...
  for ep in range(start_ep, cfg.total_epochs):
    ...
    while not epoch_done:
      ...
      if step % cfg.steps_per_log == 0:
        ...
        tri_log = (', prec {:.2%}, sm {:.2%}, '
                   'd_ap {:.4f}, d_an {:.4f}, '
                   'loss {:.4f}, tri_loss {:.4f}, ' 
                   'cent_loss {:.4f}, xent_loss {:.4f}'.format(
          prec_meter.val, sm_meter.val,
          dist_ap_meter.val, dist_an_meter.val,
          loss_meter.val, tri_loss_meter.val, cent_loss_meter.val, xent_loss_meter.val, ))
    ...
    # tri_log = (', prec {:.2%}, sm {:.2%}, '
    #            'd_ap {:.4f}, d_an {:.4f}, '
    #            'loss_combo {:.4f}, loss_tri {:.4f},  loss_cent {:.4f}, loss_xentrp {:.4f}'.format(
    #   prec_meter.avg, sm_meter.avg,
    #   dist_ap_meter.avg, dist_an_meter.avg,
    #   loss_meter.avg, cent_loss, xentrp_loss_weight, ))
